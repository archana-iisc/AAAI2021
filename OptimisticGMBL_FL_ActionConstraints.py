...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 11 01:31:40 2020
...
#LIST_NUMBER_SAMPLES = [10,20,30,40,50,60,120,150,180,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000,1500,2000]
...
for l in range(NUMBER_SIMULATIONS):
 ...
 for num_samples in LIST_NUMBER_SAMPLES:
    ...
    for s in range(N_STATES):
        for a in range(N_ACTIONS):
            for next_s in range(N_STATES):
                ...
                #BETA[s,a,next_s] = np.sqrt(1/(max(num_samples,1)))
    
    if k == len(LIST_NUMBER_SAMPLES) - 1:
        print(P_hat)
        print("printing P")
        print(P)           
    ...
    Value[k-1,l] =  abs(opt_value_LP[0,0] - value[0,0])
...
df.to_csv("Value_ConstrainedLP_GMBL_Action_10.csv",header=None)
